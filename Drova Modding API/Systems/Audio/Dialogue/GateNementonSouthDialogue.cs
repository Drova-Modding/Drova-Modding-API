...
{
    internal class GateNementonSouthDialogue : IGenericDialogueHandler
    {
        private const string GATE_NEMENTON_SOUTH = "Gate_Nemeton_South";
        public bool CanHandleDialogue(DialogueTree tree)
        {
            return tree.name.Contains(GATE_NEMENTON_SOUTH);
        }
        public void HandleDialogue(DialogueTree tree, StringBuilder dialogStringBuilder, Dictionary<string, int> actorMapping)
        {
            for (int i = 0; i < tree.allNodes.Count; i++)
            {
                ...
                string actorName = DialogueNameAndFactions.NEMENTON_GATE_SOUTH;
                dialogStringBuilder
