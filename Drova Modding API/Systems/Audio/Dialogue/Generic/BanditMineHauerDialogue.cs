...
{
    internal class BanditMineHauerDialogue : IGenericDialogueHandler
    {
        private const string BANDIT_MINE_HAUER = "DT_BanditMine_Hauer";
        public bool CanHandleDialogue(DialogueTree tree)
        {
            return tree.name == BANDIT_MINE_HAUER;
        }
        ...
        public void HandleDialogue(DialogueTree tree, StringBuilder dialogStringBuilder, Dictionary<string, int> actorMapping)
        {
            for (int i = 0; i < tree.allNodes.Count; i++)
            {
                ...
                for (int j = 0; j < DialogueNameAndFactions.BRUTUS_MINE_HAUER.Length; j++)
                {
                    string actorName = DialogueNameAndFactions.BRUTUS_MINE_HAUER[j];
                    dialogStringBuilder
