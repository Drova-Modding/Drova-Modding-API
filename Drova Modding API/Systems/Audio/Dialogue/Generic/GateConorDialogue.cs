...
{
    internal class GateConorDialogue : IGenericDialogueHandler
    {
        private const string GATE_CONOR = "DT_Gate_Conor";

        public bool CanHandleDialogue(DialogueTree tree)
        {
            return tree.name == GATE_CONOR;
        }
        ...
        public void HandleDialogue(DialogueTree tree, StringBuilder dialogStringBuilder, Dictionary<string, int> actorMapping)
        {
            for (int i = 0; i < tree.allNodes.Count; i++)
            {
                ...
                string actorName = DialogueNameAndFactions.GATE_CONOR;
                dialogStringBuilder
