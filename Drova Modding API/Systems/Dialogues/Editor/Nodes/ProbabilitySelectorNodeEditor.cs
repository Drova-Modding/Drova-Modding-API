...
namespace Drova_Modding_API.Systems.Dialogues.Editor.Nodes
{
    ...
    internal class ProbabilitySelectorNodeEditor : DrawNodeEditor
    {
        ...
        public override void DrawNode(Vector2 position)
        {
            ...
            {
                var option = _castedNode.childOptions[i].weight;
                GUI.Label(new Rect(position.x + 10, position.y + 20 + i * 20, 200, 20), $"Option {i + 1} Weight:");
                var tempWeigth = GUI.TextField(new Rect(position.x + 220, position.y + 20 + i * 20, 100, 20), option.value.ToString());
                if (float.TryParse(tempWeigth, out var result))
                {
                    option.value = result;
                }
            }
